...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for form in forms:
            # acontexts = RDRFContext.objects.filter(registry=form.registry)
            # for acontext in acontexts:
            #     # for context_form_group in acontext.context_form_group:
            #     if acontext.context_form_group is not None:
            #         print(f"Form: {form.name} Context id: {acontext.id} Context form name: {acontext.context_form_group.name}")
            ...
            for registry in patient.rdrf_registry.all():
                ...
                if registry.code in cde_values_dict:
                    for form in cde_values_dict[registry.code]:
                        jscontextvar = {}
                        form_model = RegistryForm.objects.get(name=form,registry=registry)
                        formgroupitems = ContextFormGroupItem.objects.filter(registry_form=form_model)
                        contexts = RDRFContext.objects.filter(registry=registry,
                                                              object_id=patient.id,
                                                              context_form_group__in=[formgroupitem.context_form_group for formgroupitem in formgroupitems])

                        context_ids = [context.id for context in contexts]
                        # if no context, it means we are in the case of a single context like DD registry
                        if not context_ids:
                            context_ids = [c.id for c in patient.context_models]


                        print(f"Form: {form} contexts: {context_ids}")
                        print("---------------------------------------")
                                ...
                                for cdecode in cde_values_dict[registry.code][form][section]:
                                    ...
                                    try:
                                        cdevalue = patient.get_form_value(registry_code=registry.code,
                                                                          ...
                                                                          section_code=section, context_id=context_id)
                                        # if cde is a date then display the JS format.
                                        # print(f"CDE TYPE: {cde_values_dict[registry.code][form][section][cdecode].datatype}")
                                        if cdevalue is not None and cde_values_dict[registry.code][form][section][cdecode].datatype == 'date':
                                            cdevalue = datetime.strptime(cdevalue, '%Y-%m-%d').__format__("%d-%m-%Y")
                                            # print(f"DATE: {cdevalue}")
                                        jscontextvar[cdecode] = cdevalue
                                    except KeyError:
                                        # mainly we ignore empty values.
                                        print(f"KeyError for CDE value:{patient.combined_name} {[c.id for c in patient.context_models]} | {registry.code} | {form} | {section} | {cdecode}")
